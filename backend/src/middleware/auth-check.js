...
module.exports = async (ctx, next) => {
  ...
  if (authToken instanceof Error)
    return ctx.throw(401, 'error verifying jwt');

  const user = await ctx.mongo.models.users.findOne({ _id: authToken.userId });

  if (!user) 
    return ctx.throw(401, 'account not found');
  
  const lastLogoutTime = user.lastLogoutTime;
  
  if(authToken.created < lastLogoutTime)
    return ctx.throw(401, 'account already logged out');
