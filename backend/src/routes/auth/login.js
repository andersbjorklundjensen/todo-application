...
module.exports = async (ctx) => {
  ...
  if (!password || !is.string(password)) return ctx.throw(400, 'invlaid password');


  const account = await ctx.mongo.models.users.findOne({ username });

  if (!account) return ctx.throw(400, 'account does not exist');
  ...
  if (!isPasswordValid) return ctx.throw(400, 'invalid login credentials');
