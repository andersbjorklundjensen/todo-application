...
module.exports = async (ctx) => {
  ...
  if (!is.boolean(doneStatus))
    return ctx.throw(400, 'invalid done status');

  if (!projectId || !is.string(projectId) || !mongoose.Types.ObjectId.isValid(projectId))
    return ctx.throw(400, 'invalid project id');


  const projectExists = !! (await ctx.mongo.models.projects.findOne({ _id: projectId, ownerId: ctx.state.userId }));

  if (!projectExists)
    return ctx.throw(400, 'project doesnt exist');

  const todoExists = !! (await ctx.mongo.models.todos.findOne({ _id: todoId, ownerId: ctx.state.userId }));

  if (!todoExists)
    return ctx.throw(400, 'todo doesnt exist');
